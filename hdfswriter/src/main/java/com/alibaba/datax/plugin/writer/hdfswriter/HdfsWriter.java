    ...
    public static class Job extends Writer.Job {
        ...
        private void validateParameter() {
            ...
            && !fileType.equalsIgnoreCase("PARQUET")){
                String message = "HdfsWriter插件目前只支持ORC,TEXT,PARQUET格式的文件,请将filetype选项的值配置为ORC或者TEXT或者PARQUET";
        ...
        private String buildFilePath() {
            ...
            String split = "/";
            isEndWithSeparator = this.path.endsWith(split);
...
//                tmpFilePath = String.format("%s__%s%s", userPath, tmpSuffix, IOUtils.DIR_SEPARATOR);
                tmpFilePath = String.format("%s__%s%s", userPath, tmpSuffix, split);
...
//                tmpFilePath = String.format("%s__%s%s", userPath.substring(0,userPath.length()-1), tmpSuffix, IOUtils.DIR_SEPARATOR);
                tmpFilePath = String.format("%s__%s%s", userPath.substring(0,userPath.length()-1), tmpSuffix, split);
            }
